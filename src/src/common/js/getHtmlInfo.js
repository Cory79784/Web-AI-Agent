async function getHtmlInfo() {
    ...
    const parse = (root, windowSize) => {
        ...
        function isVisible(node, bid) {
            ...
            // console.log("[wx, wy, ww, wh]", [wx, wy, ww, wh]);
            // console.log("(x + w) < wx || x > (wx + ww) || (y + h) < wy || y > (wy + wh):", (x + w) < wx || x > (wx + ww) || (y + h) < wy || y > (wy + wh));
    ...
    function ctx2tree(ctx) {
        ...
        function removeHiddenElements(doc) {
            ...
            let aEle = doc.querySelector("[href='//www.zhihu.com/zhida']");
            if (aEle) {
                const parentLi = aEle.closest("li");
                if (parentLi) {
                    parentLi.remove();
                }
            }
